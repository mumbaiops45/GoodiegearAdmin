...
export default function BannersPage() {
  ...
  const handleExport = () => {
    ...
    const headers = ['Title', 'Code', 'Placement', 'Clicks', 'Active', 'Created Date']
    const rows = b.banners.map((banner) => [
      `"${banner.title || ''}"`,
      `"${banner.code || ''}"`,
      `"${banner.placement || ''}"`,
    ...
    link.download = `banners_export_${new Date().toISOString().slice(0,10)}.csv`
    link.click()
    ...
    <div>
        ...
        action={
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={b.isFetching || b.banners.length === 0}
            ...
            <button
              onClick={b.openCreate}
              className="flex items-center gap-2 rounded-xl bg-pink-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm shadow-pink-200 hover:bg-pink-700 transition dark:shadow-pink-900/30"
            >
              <Plus className="h-4 w-4" /> Add Banner
            </button>
          </div>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <select
          value={b.filterPlace}
          onChange={(e) => b.setFilterPlace(e.target.value)}
          className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none transition focus:border-pink-400 focus:ring-2 focus:ring-pink-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
        >
          <option value="">All Placements</option>
          {(b.placements.length ? b.placements : PLACEMENTS).map((p) => (
            <option key={p} value={p}>{p}</option>
      ...
      <div className="relative z-10 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex justify-end gap-3 border-t border-slate-200 px-6 py-4 dark:border-slate-800">
          ...
          <button
            onClick={onSubmit}
            disabled={isSubmitting}
            className="flex items-center gap-2 rounded-xl bg-pink-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm shadow-pink-200 hover:bg-pink-700 disabled:opacity-60 transition dark:shadow-pink-900/30"
          >
            {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
            {editTarget ? 'Save Changes' : 'Create Banner'}
          </button>
