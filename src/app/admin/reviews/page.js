...
import {
  Star, Search, CheckCircle2, XCircle, AlertCircle,
  MessageSquare, RefreshCw, Loader2, X, Eye, Download,
} from 'lucide-react'
import useAuthStore from '@/store/authStore'
import { PageHeader, ConfirmDialog } from '@/components/ui'
import { useReviews } from '@/hooks/useReviews'
...
export default function ReviewsPage() {
  ...
  const handleExport = () => {
    ...
    const rows = r.reviews.map((review) => {
      const product = review.product?.title ?? review.productTitle ?? ''
      const customer = review.user?.name ?? review.userName ?? review.user?.email ?? ''
      const rating = Number(review.rating ?? 0)
      const comment = review.comment ?? review.text ?? ''
      const status = review.status ?? 'Pending'
        ...
        action={
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={r.isFetching || r.reviews.length === 0}
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-200 px-6 py-4 dark:border-slate-800">
            ...
            {status !== 'Published' && (
              <button
                onClick={onApprove}
                disabled={isBusy}
                className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-pink-500 to-rose-500 px-4 py-2.5 text-sm font-bold text-white hover:from-pink-600 hover:to-rose-600 disabled:opacity-50 transition"
              >
                {isBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
                Approve
              </button>
