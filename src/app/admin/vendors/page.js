...
import {
  Search, CheckCircle2, AlertCircle, Loader2,
  Store, X, RefreshCw, ShieldCheck, Clock, Mail, User, Download,
} from 'lucide-react'
import { PageHeader } from '@/components/ui'
import { useVendors } from '@/hooks/useVendors'
...
export default function VendorsPage() {
  ...
  // ─── Export to CSV ──────────────────────────────────────────────────────
  const handleExport = () => {
    if (v.vendors.length === 0) {
      alert('No vendors to export.')
      return
    }

    const headers = ['Shop Name', 'Owner', 'Email', 'Description', 'Status', 'Joined Date']
    const rows = v.vendors.map((vendor) => {
      const shopName = vendor.shopName ?? ''
      const ownerName = vendor.user?.name ?? ''
      const ownerEmail = vendor.user?.email ?? ''
      const description = vendor.shopDescription ?? ''
      const status = vendor.isApproved ? 'Approved' : 'Pending'
      const joined = vendor.createdAt
  ...
  return (
    <div>
      <PageHeader
        title="Vendors"
        subtitle={`${stats.total} registered vendors`}
        action={
          <button
            onClick={handleExport}
            disabled={v.isFetching || v.vendors.length === 0}
